...
export default function IdeaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [idea, setIdea] = useState(() => mockApi.getIdea(id));
  ...
  function handleBet(position: "YES" | "NO") {
    ...
    if (result.success) {
      setIdea(mockApi.getIdea(id) ?? idea);
    }
  }
  ...
  return (
    <div className="px-8 py-8 max-w-4xl">
      ...
      <p className="text-[#8b8d97] text-sm mb-6">
        Zgłoszone przez{" "}
        <span className="text-white font-medium">{idea.creator_username}</span>
        {" · "}{formatDate(idea.created_at)}
      </p>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Left col: description + hat scores */}
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-xl p-5">
            <h2 className="text-indigo-300 text-xs font-semibold uppercase tracking-wider mb-2">
              Milestone do osiągnięcia
            </h2>
            <p className="text-white text-sm font-medium">{idea.milestone}</p>
            <p className="text-[#8b8d97] text-xs mt-1">
              do {formatDate(idea.market_closes_at)}
            </p>
          </div>
          ...
          {idea.hat_scores.length > 0 && (
            <div className="bg-[#13141a] border border-[#1e2028] rounded-xl p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-[#8b8d97] text-xs bg-[#1e2028] px-2.5 py-1 rounded-lg">
              ...
              <div className="mb-4">
                <div className="flex justify-between text-xs mb-1">
                  ...
                  <span className={cn(
                    "font-semibold",
                    idea.validation_score >= 36 ? "text-emerald-400" : "text-red-400"
                  )}>
                ...
                <div className="h-2 bg-[#1e2028] rounded-full overflow-hidden">
                  <div
                    className={cn(
                      ...
                      idea.validation_score >= 36 ? "bg-emerald-500" : "bg-red-500"
                    )}
                    style={{ width: `${(idea.validation_score / 60) * 100}%` }}
                  />
                </div>
                <div className="relative h-0">
                  <div className="absolute border-l-2 border-dashed border-[#4b4d58] h-4 -top-3" style={{ left: "60%" }} />
                  <span className="absolute text-[9px] text-[#6b6d77] -top-5 -translate-x-1/2" style={{ left: "60%" }}>próg 36</span>
                </div>
              </div>
              ...
              <div className="space-y-2 mt-6">
                {idea.hat_scores.map((hs) => {
                  ...
                  return (
                    <button
                      key={hs.hat_color}
                      onClick={() => setExpandedHat(isExpanded ? null : hs.hat_color)}
                      className="w-full text-left"
                    >
                      <div className="flex items-center gap-3 p-3 rounded-lg bg-[#1a1c22] hover:bg-[#1e2028] transition-colors">
                        <span className={cn("text-xs font-semibold px-2 py-0.5 rounded-md min-w-[80px] text-center", cfg.color)}>
                          {cfg.label}
                        </span>
                        ...
                        <svg
                          className={cn("w-3.5 h-3.5 text-[#6b6d77] transition-transform shrink-0", isExpanded && "rotate-180")}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#13141a] border border-[#1e2028] rounded-xl p-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs">
                <span className="text-[#8b8d97]">Wolumen</span>
                <span className="text-white font-medium">{formatCoins(totalVolume)}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-[#8b8d97]">Pula TAK</span>
                <span className="text-emerald-400 font-medium">{formatCoins(idea.yes_pool)}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-[#8b8d97]">Pula NIE</span>
                <span className="text-red-400 font-medium">{formatCoins(idea.no_pool)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#13141a] border border-[#1e2028] rounded-xl p-4">
            ...
            <div className="flex items-center justify-between mb-3">
              <span className="text-[#8b8d97] text-xs">Twoje saldo</span>
              <span className="text-white text-xs font-medium">{user.coin_balance} 🪙</span>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="flex items-center gap-2 bg-[#1a1c22] border border-[#2d2f3a] rounded-lg p-2">
                <button
                  ...
                  className="w-6 h-6 flex items-center justify-center text-[#8b8d97] hover:text-white text-lg leading-none"
                >
                  −
                </button>
                <input
                  ...
                  value={betAmount}
                  onChange={(e) => setBetAmount(Math.min(user.coin_balance, Math.max(1, Number(e.target.value))))}
                  className="flex-1 bg-transparent text-white text-sm font-medium text-center outline-none"
                />
                <button
              ...
              <div className="flex gap-1.5 mt-2">
                {[5, 10, 25, 50].map((amt) => (
                  <button
                    key={amt}
                    onClick={() => setBetAmount(Math.min(user.coin_balance, amt))}
                    className="flex-1 text-[10px] py-1 rounded bg-[#1e2028] text-[#8b8d97] hover:text-white hover:bg-[#2a2c36] transition-colors"
                  >
                    {amt}
                  </button>
                ))}
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-3 rounded-xl text-sm font-bold transition-all bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500 hover:text-white hover:border-emerald-500"
              >
                TAK {prob}¢
              </button>
              <button
                onClick={() => handleBet("NO")}
                className="flex-1 py-3 rounded-xl text-sm font-bold transition-all bg-red-500/10 text-red-400 border border-red-500/20 hover:bg-red-500 hover:text-white hover:border-red-500"
              >
                NIE {100 - prob}¢
              </button>
            </div>

            <p className="text-[#6b6d77] text-[10px] text-center mt-2">
              Stawiasz {betAmount} 🪙 · Możliwa wyplata zależy od puli
            </p>
          </div>
        </div>
      </div>
